...
def create_model(
    *,
    ...
    if model_name == 'regression':
        ...
        model = ViewportTransformerMTIO(
            in_channel=2,
            fut_window=fut_window,
            d_model=hidden_dim if hidden_dim is not None else 512,
            dim_feedforward=hidden_dim if hidden_dim is not None else 512,
            num_encoder_layers=block_num if block_num is not None else 2,
            num_decoder_layers=block_num if block_num is not None else 2,
            device=device,
            seed=seed
        )
        ...
        model = ViewportPretrainedLM(
            in_channels=2,
            fut_window=fut_window,
            d_model=hidden_dim if hidden_dim is not None else 768,
            rank=rank if rank is not None else 128,
            conv_channels=conv_channels if conv_channels is not None else 128,
            plm_type="bert",
            n_layer=n_layer if n_layer is not None else 12,
            device=device
        )
...
def run(args, config):
    ...
    file_prefix = f'{args.model}{"_conv_" + str(args.conv_channels) if args.conv_channels is not None else ""}'\
                  f'{"_ln_" + str(args.layer_num) if args.layer_num is not None else ""}'\
                  f'{"_rank_" + str(args.rank) if args.rank is not None else ""}'\
                  f'{"_hid_" + str(args.hidden_dim) if args.hidden_dim is not None else ""}'\
                  f'_his_{args.his_window}_fut_{args.fut_window}_ss_{args.sample_step}'\
                  f'_epochs_{args.epochs}_bs_{args.bs}_lr_{args.lr}_seed_{args.seed}'
    model = create_model(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conv-channels', type=int, help='Number of kernels in feature encoder.(for plm models)')
    parser.add_argument('--layer-num', type=int, help='Number of encoder layers. (for plm models)')
    parser.add_argument('--hidden-dim', type=int, help='Number of hidden dimensions')
    parser.add_argument('--block-num', type=int, help='Number of encoder/decoder blocks')
    ...
    parser.add_argument('--rank', action="store", dest='rank', help='LORA matrices rank.', type=int)
